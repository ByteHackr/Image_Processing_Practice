...
hist1 = np.zeros(len(hist),dtype=int)


for i in range(0,len(hist)-2):
    if i == 0:
        hist1[i]=(hist[i]+hist[i+1]+hist[i+2])/3
    elif i == 1:
        hist1[i]=(hist[i]+hist[i+1]+hist[i+2])/3
    else:
        hist1[i]=(hist[i-2]+hist[i-1]+hist[i]+hist[i+1]+hist[i+2])/5

hist1[254] = (hist[253]+hist[254]+hist[255])/3

hist1[255] = (hist[253]+hist[254]+hist[255])/3
