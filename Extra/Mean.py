...
img2 = np.array([])
img2 = cv2.imread('CRW_4901_JFRtamp25.jpg')
...
for i in range(row):
    for j in range(col):
        x =  img2[i][j][0]*0.07+img2[i][j][1]*0.72+img2[i][j][2]*0.21
        img1[i][j] = x
        #output[i][j] = x
...
output_v_neg = 255 - output_v
output_v_n_o = (img1+output_v_neg)/2

cv2.imwrite('neg_Variance+Org.jpg',output_v_n_o)